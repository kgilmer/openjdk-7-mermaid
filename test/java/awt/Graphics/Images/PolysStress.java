...
/*
 * @test
 * @summary Builds on testcase Polys02, adding XOR and LiveResize into the mix.
 * @summary <rdar://problem/4969413> [JavaJDK15] g.fillPolygon + resize -> CGContextGetClipBoundingBox: invalid context
 * @summary com.apple.junit.java.graphics.images
 * @library ../../regtesthelpers
 * @build LiveResizeAction MovingPoints
 * @run main PolysStress
 */

import test.java.awt.regtesthelpers.LiveResizeAction;
...
public class PolysStress extends TestCase {
    static boolean TESTXOR = true;
    ...
    public void testPolyWithXORandLiveResize() throws Exception {
        final AnimatedWindow testWindow = new AnimatedWindow(rect);

        // Attach a resizer to the Frame
        //LiveResizeThread lrt = new LiveResizeThread(testWindow, LiveResizeThread.VIARIABLE_MOVE);         
        //lrt.start();
        final LiveResizeAction.VariableMove move = new LiveResizeAction.VariableMove(testWindow, new Robot());
        move.run();     
        ...
        if (!RUNLONG) {
            ticktock.schedule(stopper, 4250L);
        }
        else {
            ticktock.schedule(stopper, 30000L);
        }

        //while(!done || !lrt.isDone()) {
        //  Thread.sleep(250);
        //}
    ...
    public static Test suite() {
        return new TestSuite( PolysStress.class);
    }
