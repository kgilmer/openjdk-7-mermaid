...
public class VisibilityValidator {
    static final int SETUP_PERIOD = 5000; // Wait up to five seconds for our window events
    ...
    //
    // The only way to make it through this routine is for the window to generate BOTH
    // a windowOpened, a windowActivated event and a windowGainedFocus, or to timeout.
    //
    ...
    // utility function that waits until a Component is shown with the appropriate color
    ...
    // utility function that waits for 5 seconds for Component to be shown with the appropriate color
    ...
    // utility function that waits until a Component is up with the appropriate color
    ...
    // utility function that waits until specific screen coords have the appropriate color
    ...
    public static boolean colorMatch(final Color actual, final Color expected) {
        //System.out.println("colorMatch actual: " + actual + " expect: " + expected);
        ...
        // check to see if the brightness was so low or so high that the hue got clamped to red
        if (brightnessMatched && !hueMatched) {
            return (expectedBrightness < 0.15f);
        }
        
        //System.out.println("colorMatched? " + (brightnessMatched && hueMatched));
    ...
    // matches hues from 0.0 to 1.0, accounting for wrap-around at the 1.0/0.0 boundry
    static boolean closeMatchHue(final float actual, final float expected, final float tolerance) {
        if (closeMatch(actual, expected, tolerance)) return true;
    ...
    static boolean closeMatch(final float actual, final float expected, final float tolerance) {
        //System.out.println("\tcloseMatch actual: " + actual + " expect: " + expected + " tolerance: " + tolerance);
        //System.out.println("\tcloseMatch " + ((expected + tolerance) > actual) + " && " + ((expected - tolerance) < actual));
