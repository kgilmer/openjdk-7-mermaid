...
- (id) initWithPeer:(jobject)thePeer withEnv:(JNIEnv *)env{
  peer = JNFNewGlobalRef(env, thePeer);
  ...
  button = [[AWTNSButton alloc] initWithTrayIcon:self withEnv:env];
...
(JNIEnv *env, jobject peer){
  AWTTrayIcon *trayIcon = [[AWTTrayIcon alloc] initWithPeer:peer withEnv:env];
...
(JNIEnv *env, jobject self, jlong model, jstring tooltip){
  AWTTrayIcon *icon = jlong_to_ptr(model);
  [icon setTooltip:JNFJavaToNSString(env, tooltip)];
}
