...
{
    public static final LazySingleton<Dynamic> fDynamic =
        new LazySingletonFromDefaultContructor<Dynamic>(Dynamic.class);
    static public AquaButtonBorder getDynamicButtonBorder() {
        return fDynamic.get();
    }
    
    private static final LazySingleton<Toggle> fToggle =
        new LazySingletonFromDefaultContructor<Toggle>(Toggle.class);
    static public AquaButtonBorder getToggleButtonBorder() {
        return fToggle.get();
    }
    
    public static LazySingleton<Toolbar> fToolBar =
        new LazySingletonFromDefaultContructor<Toolbar>(Toolbar.class);
    static public Border getToolBarButtonBorder() {
        return fToolBar.get();
    }
    
    public static final LazySingleton<Named> fBevel =
        new LazySingleton<Named>() {
        protected Named getInstance() {
            return new Named(Widget.BUTTON_BEVEL,
                             new SizeDescriptor(new SizeVariant().alterMargins(2, 4, 2, 4)));
        }
    };
    ...
    {
        // for now we don't paint a border; we let the button
        // paint it since there needs to be a strict ordering for
        // aqua components.
        //paintButton(c, g, x, y, width, height);
    }
    
    public void paintButton(final Component c, final Graphics g,
                            int x, int y, int width, int height)
    {
        ...
        painter.state.set((state != State.DISABLED && state != State.INACTIVE) &&
                        b.isFocusPainted() &&
                        isFocused(b) ? Focused.YES : Focused.NO);
                                   ...
                                   final ButtonModel model)
    {
        ...
        // The default button shouldn't draw its color when the window
        // is inactive.
        // Changed for <rdar://problem/3614421>: Aqua LAF Buttons are incorrectly drawn disabled
        // All we need to do is make sure we aren't the default button
        // any more and that we aren't active, but we still are enabled
        // if the button is enabled.  If we set dimmed we would appear
        // disabled despite being enabled and click through works so
        // this now matches the text drawing and most importantly the HIG
        ...
        if (model.isSelected() && isSelectionPressing()) {
            return State.PRESSED;
        }
    ...
    public Insets getBorderInsets(final Component c) {
        ...
        margin = (margin == null) ?
            new InsetsUIResource(0, 0, 0, 0) : (Insets)margin.clone();
    ...
    public void alterPreferredSize(final Dimension d) {
        if (sizeVariant.h > 0 && sizeVariant.h > d.height) {
            d.height = sizeVariant.h;
        }
        if (sizeVariant.w > 0 && sizeVariant.w > d.width) {
    ...
    public static class Dynamic extends AquaButtonBorder {
                                       ...
                                       final ButtonModel model)
        {
            ...
            painter.state.set(state == State.PULSED ?
                              Animating.YES : Animating.NO);
            return state;
        }
        
        public Insets getContentInsets(final AbstractButton b,
                                       final int width, final int height)
        {
        ...
        protected Widget getStyleForSize(final AbstractButton b,
                                         final Size size,
                                         final int width, final int height)
        {
            ...
            if (height < SizeConstants.fSquareButtonHeightThreshold ||
                width < SizeConstants.fSquareButtonWidthThreshold)
            {
                return Widget.BUTTON_BEVEL;
            }
            
            if (height <= SizeConstants.fNormalButtonHeight + 3 &&
                width < SizeConstants.fNormalMinButtonWidth)
            {
                return Widget.BUTTON_BEVEL;
            }
            
            if ((height > SizeConstants.fNormalButtonHeight + 3) ||
                (b.getIcon() != null) || hasSmallerInsets(b))
            {
    ...
    public static class Toolbar extends AquaButtonBorder {
        ...
        {
            if (!model.isSelected()) {
                // only paint when the toolbar button is selected
                return;
            }
