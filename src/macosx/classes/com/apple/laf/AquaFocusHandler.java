 ...
 * AquaTextFieldUI and AquaTextPaneUI to control painting of the
 * component's border.
 * NOTE: It is assumed that this handler is added to components
 * that extend JComponent.
 */
public class AquaFocusHandler implements FocusListener, PropertyChangeListener {
    // Flag to help focusGained() determine whether the origin focus loss
    // was due to a temporary focus loss or not.
    ...
    public void focusGained(final FocusEvent ev) {
        // If we gained focus and it wasn't due to a previous temporary
        // focus loss or the frame became active again, then repaint the
    ...
    public void focusLost(final FocusEvent ev) {
        ...
        // If we lost focus due to a permanent focus loss then repaint
        // the border on the component.
        if (!wasTemporary) {
            AquaBorder.repaintBorder((JComponent)ev.getSource());
        }
    }
    ...
    public void propertyChange(final PropertyChangeEvent ev) {
        ...
        if (Boolean.TRUE.equals(ev.getNewValue())) {
            // The FRAME_ACTIVE_PROPERTY change event is sent before a
            // component gains focus.
            // We set a flag to help the focusGained() determine when
            // they should be repainting the component's focus.
            repaintBorder = true;
        } else if (wasTemporary) {
            // The FRAME_ACTIVE_PROPERTY change event is sent after a
            // component loses focus.
            // We use the wasTemporary flag to determine if we need to
            // repaint the border.                
            AquaBorder.repaintBorder((JComponent)ev.getSource());
        }
    }
    ...
    protected static boolean isActive(final JComponent c) {
        ...
        final Object activeObj =
            c.getClientProperty(AquaFocusHandler.FRAME_ACTIVE_PROPERTY);
        if (Boolean.FALSE.equals(activeObj)) {
            return false;
        }
        return true;
    }
